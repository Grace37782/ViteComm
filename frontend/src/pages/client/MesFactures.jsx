    ...
    return (
      <div className="w-full min-h-screen font-sans flex items-center justify-center" style={{ background: 'var(--bg)' }}>
        <div className="text-center">
          ...
          <div className="text-sm" style={{ color: 'var(--text-muted)' }}>Chargement des factures…</div>
        </div>
      </div>
    )
  }
    ...
    <div className="w-full min-h-screen font-sans mx-auto max-w-3xl" style={{ background: 'var(--bg)', paddingBottom: 80 }}>
      ...
      <div className="relative overflow-hidden px-5 pt-5 pb-5"
        ...
        <div className="relative z-10 flex items-center gap-3">
          ...
          <div>
            <div className="text-white font-black text-base">Mes factures</div>
            <div className="text-white/70 text-xs">{factures.length} facture{factures.length !== 1 ? 's' : ''}</div>
          </div>
        </div>
      </div>
                  ...
                  <button key={f.id_facture} onClick={() => setDetailFacture(f)}
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        ...
                        <div className="text-[10px]" style={{ color: 'var(--text-muted)' }}>Commande #{f.id_commande}</div>
                      </div>
                      <span className="text-xs font-bold px-2.5 py-1 rounded-full"
                        ...
                        {isPaid ? <><CheckCircle size={12} className="inline" /> Payé</> : <><Loader2 size={12} className="inline" /> En attente</>}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>{new Date(f.date_emission).toLocaleDateString('fr-FR')} · {f.articles.length} articles</span>
          ...
          <>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs" style={{ color: 'var(--text-muted)' }}>Commande #{detailFacture.id_commande}</div>
              </div>
            </div>
            ...
            <div className="rounded-2xl p-5" style={{ background: 'var(--surface)', border: '1.5px solid var(--border)' }}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <div className="font-black text-sm" style={{ color: 'var(--text-primary)' }}>Articles</div>
              </div>
            ...
            <div className="rounded-2xl p-5" style={{ background: 'var(--surface)', border: '1.5px solid var(--border)' }}>
              ...
              <div className="space-y-1.5 text-xs">
                <div className="flex justify-between">
                  <span style={{ color: 'var(--text-muted)' }}>Marchandises</span>
            ...
            {detailFacture.paiement && (
              <div className="rounded-2xl p-4" style={{ background: isDark ? 'rgba(45,196,145,0.1)' : '#E1F5EE', border: `1.5px solid ${isDark ? 'rgba(45,196,145,0.3)' : '#9FE1CB'}` }}>
                ...
                <div className="flex flex-col gap-1 text-xs">
                  ...
                  <div className="flex justify-between">
                    <span style={{ color: '#0F6E56' }}>Montant</span>
