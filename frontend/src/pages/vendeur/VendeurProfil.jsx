...
const MOTIFS_REPUTATION = [
  ...
  { label: 'Qualité', icon: '⭐', description: 'Conformité des produits vendus' },
  { label: 'Communication', icon: '💬', description: 'Réactivité et courtoisie' },
  { label: 'Fiabilité', icon: '🤝', description: 'Honnêteté et transparence' },
]
          ...
          {!editing ? (
            <div className="rounded-2xl p-6" style={{ background: 'var(--surface)', border: '1.5px solid var(--border)' }}>
              ...
              <div className="text-center mb-5">
                ...
                <p className="text-sm font-semibold mt-1" style={{ color: '#BA7517' }}>📦 Vendeur ViteComm</p>
                {form.nom_etablissement && (
                  <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>🏪 {form.nom_etablissement}</p>
                )}
              </div>

              <div className="flex flex-col gap-3 mb-5">
                <InfoRow label="Établissement" value={form.nom_etablissement || '—'} icon="🏪" />
                <InfoRow label="Marché" value={form.localisation_marche || '—'} icon="📍" />
                <InfoRow label="Email" value={profile?.email} icon="✉️" />
                <InfoRow label="Téléphone" value={profile?.telephone || '—'} icon="📱" />
                <InfoRow label="Score réputation" value={`${scoreReputation}/5`} icon="⭐" />
            ...
            <div className="rounded-2xl p-6" style={{ background: 'var(--surface)', border: '1.5px solid var(--border)' }}>
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-semibold" style={{ color: 'var(--text-muted)' }}>🏪 Nom de l'établissement</label>
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-semibold" style={{ color: 'var(--text-muted)' }}>📍 Localisation du marché</label>
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-2xl p-6 text-center"
            ...
            <div className="flex justify-center gap-1 mt-3">
              {[1, 2, 3, 4, 5].map(s => (
                <span key={s} className="text-2xl" style={{ opacity: s <= scoreReputation ? 1 : 0.3 }}>⭐</span>
              ))}
            </div>
          </div>
